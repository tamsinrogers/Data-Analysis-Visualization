...
class Analysis:
	...
	def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
		...
		for i in range(varLength):						# repeat variable # times
			x = self.data.data[:, indList[i]]			# get all the data from the right columns
						
			for j in range(varLength):					# repeat variable # times
				y = self.data.data[:, indList[j]]		
				ax[i][j].scatter(x,y, color=colors[rand.randint(0,9)])	# make the scatter plot
				count += 1												# next coordinates
